...
public class GenreService {
    ...
    public Optional<Genre> updateGenre(Long id, Map<String, Object> updates) {
        ...
        if (updates.containsKey("movies") && updatedGenre.getMovies().isEmpty()) { // If the "movies" field exists in
                                                                                   // the given JSON body and it is
                                                                                   // empty then it clears the
                                                                                   // associations
            existingGenre.getMovies().clear();
        } else if (!updatedGenre.getMovies().isEmpty()) { // If the "movies" field isnt empty in the JSON body then
                                                          // checks if the movies exist
            Set<Movie> updatedMovies = new HashSet<>();
            for (Movie movie : updatedGenre.getMovies()) { // Check if the given associated movies exist and throw an
                                                           // exception if not
                Movie existingMovie = movieRepository.findById(movie.getId())
                        .orElseThrow(() -> new ResourceNotFoundException("Movie not found with id: " + movie.getId()));
    ...
    // Method to delete a genre and throw an exception if it isn't found or it has
    // associated movies
    @Transactional
    public void deleteGenre(Long id, boolean force) {
        ...
        if (!force && !genre.getMovies().isEmpty()) {
            throw new IllegalStateException("Cannot delete genre '" + genre.getName() + "' because it has "
