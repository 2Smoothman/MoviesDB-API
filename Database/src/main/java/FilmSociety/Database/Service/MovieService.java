package FilmSociety.Database.Service;

import FilmSociety.Database.Entity.Movie;
import FilmSociety.Database.Entity.Actor;
import FilmSociety.Database.Entity.Genre;
import FilmSociety.Database.Exception.ResourceNotFoundException;
import FilmSociety.Database.Repository.ActorRepository;
import FilmSociety.Database.Repository.GenreRepository;
import FilmSociety.Database.Repository.MovieRepository;
...
public class MovieService {
    ...
    public Page<Actor> getAllActorsInMovie(Long movieId, Pageable pageable) {
        ...
        Page<Actor> actors = actorRepository.findByMoviesId(movieId, pageable); // If movie exists then calls the actor
                                                                                // list from the actor repository
    ...
    public Optional<Movie> updateMovie(Long id, Map<String, Object> updates) {
        Movie existingMovie = movieRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Movie not found with id: " + id));
        /*
         * Using objectmapper to convert the input query's JSON body to a map and use it
         * to confirm if the user is trying to clear the existing
        ...
        if (updates.containsKey("genres") && updatedMovie.getGenres().isEmpty()) { // If the "genres" field exists in
                                                                                   // the given JSON body and its empty
                                                                                   // then it clears associations
        ...
        if (updates.containsKey("actors") && updatedMovie.getActors().isEmpty()) { // If the "actors" field exists in
                                                                                   // the given JSON body and its empty
                                                                                   // then it clears associations
    ...
    // Method to delete a movie and throw an exception if movie isn't found or it
    // has associated genres or actors
    @Transactional
    public void deleteMovie(Long id, boolean force) {
        ...
        if (!force && (!movie.getGenres().isEmpty() || !movie.getActors().isEmpty())) {
            throw new IllegalStateException(
                    "Unable to delete movie '" + movie.getTitle() + "' because it has associated genre(s) or actor(s)");
        }
        if (force) { // If force=true then removes the movie from all the associated genres and
                     // actors
            for (Genre genre : movie.getGenres()) {
