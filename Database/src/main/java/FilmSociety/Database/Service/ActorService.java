...
public class ActorService {
    ...
    public Optional<Actor> updateActor(Long id, Map<String, Object> updates) {
        ...
        if (updates.containsKey("movies") && updatedActor.getMovies().isEmpty()) { // If the "movies" field exists in
                                                                                   // the given JSON body and it is
                                                                                   // empty then it clears the
                                                                                   // associations
            existingActor.getMovies().clear();
        } else if (!updatedActor.getMovies().isEmpty()) { // If the "movies" field isnt empty in the JSON body then
                                                          // checks if the movies exist
            Set<Movie> updatedMovies = new HashSet<>();
            for (Movie movie : updatedActor.getMovies()) { // Check if the associated movies exist and throw an
    ...
    // Method to delete an actor and throw an exception if actor isn't found or they
    // have associated movies
    @Transactional
    public void deleteActor(Long id, boolean force) {
        Actor actor = actorRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Actor not found with id: " + id));
        if (!force && !actor.getMovies().isEmpty()) {
            throw new IllegalStateException("Unable to delete actor '" + actor.getName() + "' because they have "
                    + actor.getMovies().size() + " associated movie(s)");
        }
